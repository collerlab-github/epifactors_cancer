...
cancer <- 'WT'
dir.create(paste0("../../",project,"_", cancer, "/metapcna"), showWarnings = F)
setwd(paste0("../../",project,"_", cancer, "/metapcna"))
...
# read full gene expressiond data
print('Reading full log2 normalized gene expression data')
log2_counts <- read.csv(paste0('../RNA-seq_datasets/',cancer,'_log2normalized_counts.csv'), row.names=1)
...
# add metapcna factor
metapcna <- read.csv('../../metadata/meta-pcna_genes.csv', header=F) %>% filter(V1%in%rownames(hugo_df))
metapcna_factor <- apply(hugo_df[metapcna$V1,], MARGIN=2, FUN = median)
...
# add metaPCNA median expression to correct for cell cycle effects
patient_info[names(metapcna_factor),'metapcna'] <- metapcna_factor

# subset epigenes to only those that are significantly prognostic after cox correction

sig_genes <- read.csv(paste0('../05_gene_clinical_prediction_analysis/',cancer,'_significant_cox_cluster_differential_genes_',end,'.csv'),row.names=1) %>%
  filter(adjpval_cluster<=0.05)
sample_cluster <- sample_cluster[,rownames(sig_genes)]
...
if (project=='TCGA'){
  ...
  # sample age and gender and metapcna
  sample_age <- patient_info[,'age',F]
  sample_gender <- patient_info[,'gender',F]
  sample_metapcna <- patient_info[,'metapcna',F]
  # read survival data
  ...
  cancer_surv$metapcna <- sample_metapcna[cancer_surv$bcr_patient_barcode,]
  ...
  # extract samples from survival data
  cancer_surv <- patient_info[which(rownames(patient_info)%in%rownames(sample_cluster)),c("OS.time","OS","age","gender","metapcna")]
...
category_df <- cbind(cancer_surv[rownames(category_df),c("age","gender",'metapcna')],category_df)
...
pval_df <- data.frame(row.names = colnames(category_df)[6:length(colnames(category_df))],pval=rep(0,length(colnames(category_df))-5)) 
# initialize cox regression pvalue df and hazard ratio effect size
cox_df <- data.frame(row.names = colnames(category_df)[6:length(colnames(category_df))],cluster=rep(0,length(colnames(category_df))-5), age=0, gender=0, metapcna=0)
cox_HR <- data.frame(row.names = colnames(category_df)[6:length(colnames(category_df))],cluster=rep(0,length(colnames(category_df))-5), age=0, gender=0, metapcna=0) 
...
pval_func <- function(gene, category_df){
  ...
  row_na <- apply(category_df[,c('age','gender','metapcna',gene)],1, function(x) sum(is.na(x)))
  ...
  if (low_no_cox | high_no_cox){
    ...
    res.cox <- coxph(s ~ category_df[,gene] + metapcna, data =  category_df)
    coxpval <- c(coef(summary(res.cox))[1,5],'NA','NA',coef(summary(res.cox))[2,5])
    cox_df[gene,] <<- coxpval
    coxHR <- c(coef(summary(res.cox))[1,2],'NA','NA', coef(summary(res.cox))[2,2])
    cox_HR[gene,] <<- coxHR
  }
    ...
    #     return(NULL)
    #   })
    res.cox <- coxph(s ~ category_df[,gene] + age + gender +metapcna, data =  category_df)
    coxpval <- coef(summary(res.cox))[,5]
    cox_df[gene,] <<- coxpval
    coxHR <- coef(summary(res.cox))[,2]
    cox_HR[gene,] <<- coxHR
  }
  else{ # run cox regression on just cluster and age and metapcna
    res.cox <- coxph(s ~ category_df[,gene] + age + metapcna, data =  category_df)
    coxpval <- c(coef(summary(res.cox))[1:2,5],'NA',coef(summary(res.cox))[3,5])
    cox_df[gene,] <<- coxpval
    coxHR <- c(coef(summary(res.cox))[1:2,2],'NA',coef(summary(res.cox))[3,2])
    cox_HR[gene,] <<- coxHR
  }
}
...
cox_df$adjpval_metapcna <- p.adjust(cox_df$metapcna, method='BH')
...
sig_pval <- sig_pval[order(sig_pval$adjpval_cluster),]
...
sig_metapcna <- cox_df[rownames(sig_pval),c("metapcna","adjpval_metapcna")]
write.csv(sig_metapcna, file = paste0(cancer,"_significant_cox_metapcna_differential_genes_",end,".csv"), quote=F, row.names=T)
...
write.csv(sig_epigenes_df, file=paste0('../RNA-seq_datasets/',cancer,'_metapcna_',tolower(end),'_epigenes_log2_counts.csv'))
