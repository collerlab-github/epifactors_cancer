...
a = read.table("Wmodules.txt", header=T, sep='\t', check.names=F) #input WGCNA output file 

b = a[,c("GENE", "MODULE")]
colnames(b)= c("Symbol", "module")
write.table(b, file="WGCNA.scad.coexpr.modules.txt", sep = "\t", row.names = F, col.names = T, quote = F)
...
minputfname= "WGCNA.scad.coexpr.modules.txt"
##################################
##################################
