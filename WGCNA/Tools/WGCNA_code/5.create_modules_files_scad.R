setwd("~/Desktop/SCAD_Dseq/WGCNA_process_dseq_scad/sft_threshold7")
...
msd <- coexp_modules[,c(1,3,4)]
msd$MODULE <- gsub(pattern="WGCNA_",replacement="",x=msd$MODULE) # remove WGCNA_
write.table(msd, file="dseq_scad_info.txt", sep='\t', row.names=F, col.names=T, quote=F)

#concatenate functional annotations of pathway colors
library(dplyr)
dseq_all_info <- read.delim("dseq_scad_info.txt", header=TRUE)

# filter(.data=dseq_all_info, MODULE=='paleturquoise', SOURCE=='canonical')
# find unique module and source combos
dseq_groups <- dseq_all_info[!duplicated(dseq_all_info[,c(1,2)]),c(1,2)]
...
for (modsource in dseq_groups$combined){
  ...
  descriptions <- filter(dseq_all_info, MODULE==mod, SOURCE==source)$DESCR
...
write.table(result_dseq, file="dseq_scad_WGCNA_modules_combineDescr.txt",quote=FALSE,sep="\t",row.names=FALSE,col.names=TRUE)

###################################################
# modify Wmodules file
wmodules <- read.delim("Wmodules.txt", header = TRUE)
wmodules$MODULE <- gsub(pattern="WGCNA_", replacement="", x=wmodules$MODULE)
write.table(wmodules,file="dseq_scad_WGCNA_modules.txt",quote=FALSE,sep="\t",row.names=FALSE,col.names=TRUE)
