...
dir.create(paste0("../../",project,"_",cancer,"/random_prognostic"), showWarnings = F)
setwd(paste0("../../",project,"_", cancer, "/random_prognostic"))

# number of epigenes
epigene_file <- list.files(path='../RNA-seq_datasets/',pattern=paste0(cancer, '_epigenes_log2norm_sd0\\.\\d+'), full.names = T)
num_epigenes <- nrow(read.csv(epigene_file))
sd_cutoff <- as.numeric(gsub('.*?(\\d.\\d)_counts.csv','\\1',epigene_file))
set.seed(0)
# perform sd cutoff for each cancer type
random_genes <- sample(read.csv("../../Summary_tables/random_prognostic/random_genes.csv")[,1], num_epigenes, replace = F)
...
sample_cluster <- filter_func(log2_counts)
sample_cluster <- as.data.frame(t(sample_cluster[random_genes,]))
gene_sds <- apply(sample_cluster, 2,sd)
sample_cluster <- sample_cluster[,names(gene_sds)[gene_sds>=sd_cutoff]]
