...
print(args)
# args <- list(project='TCGA',cancer='PAAD',rank=2)
...
if(project=="TCGA"){
  ...
  # write to file
  write.csv(cancer_surv[,c("OS","OS.time","DSS","DSS.time","PFI","PFI.time","cluster","age","gender")], paste0('Rank_',rank,'/',project,'_',cancer,'_survival_info.csv'),row.names=T,quote=F)
...
plot_surv <- function(clust, cancer_surv){
  ...
  for (e in endpoint){
    ...
    ggsave(file = paste0('Rank_',rank,'/',cancer,"_rank",rank,"_cluster", paste(clust, collapse="_"), "_",e,".png"))
  }
}
# plot curves for each pair of clusters
l <- lapply(cluster_comb, FUN=plot_surv, cancer_surv=cancer_surv)
write.csv(pval_df,file = paste0('Rank_',rank,'/endpoint_pval.csv'),row.names=T, quote=F)
write.csv(cox_df, file = paste0('Rank_',rank,'/cox_pval.csv'),row.names=T, quote=F)
