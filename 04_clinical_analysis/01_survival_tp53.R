...
tp53_info <- read.table(file=paste0('../06_diff_gene_info/',cancer,'_tp53_mutation_patients.tsv'),sep='\t',header=T,row.names=3)[,'Altered',F]
...
if(project=="TCGA"){
  ...
  cancer_surv$p53 <- tp53_info[cancer_surv$bcr_patient_barcode,'Altered']
  ...
  # write to file
  write.csv(cancer_surv[,c("OS","OS.time","DSS","DSS.time","PFI","PFI.time","cluster","age","gender",'p53')], paste0('Rank_',rank,'/',project,'_',cancer,'_survival_info.csv'),row.names=T,quote=F)
...
rownames(cox_df) <- c('cluster','age','gender','p53')
...
plot_surv <- function(clust, cancer_surv){
  ...
  for (e in endpoint){
    ...
    if (groupwise==T){
      ...
      e_data <- cancer_surv[,c(e,t,'cluster','age','gender','p53')]
    }
    ...
    if(length(unique(e_clust_data$gender))>1){
      res.cox <- coxph(s ~ cluster + age + gender + p53, data =  e_clust_data)
      coxpval <- coef(summary(res.cox))[,5]
      cox_df[,e] <<- coxpval
    }
    else{
      res.cox <- coxph(s ~ cluster + age + p53, data =  e_clust_data)
    ...
    ggsave(file = paste0('Rank_',rank,'/',cancer,"_rank",rank,"_cluster", paste(clust, collapse="_"), "_",'_p53', "_",e,".png"))
  }
}
# plot curves for each pair of clusters
l <- lapply(cluster_comb, FUN=plot_surv, cancer_surv=cancer_surv)
write.csv(pval_df,file = paste0('Rank_',rank,'/endpoint_pval_p53.csv'),row.names=T, quote=F)
write.csv(cox_df, file = paste0('Rank_',rank,'/cox_pval_p53.csv'),row.names=T, quote=F)
