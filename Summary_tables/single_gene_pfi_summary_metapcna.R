...
dir.create('metapcna',showWarnings = F)
setwd('metapcna/')
library(xlsx)
library(dplyr)
...
# create cancer x epigene endpoint cox pvalue data frames for cluster, age, gender, and metapcna 
cluster_df <- pval_df
age_df <- pval_df
gender_df <- pval_df
metapcna_df <- pval_df
...
get_sig_genes <- function(filename, adj_pval_column_num=2){
  endpoint_pvals <- read.csv(filename, header=T, row.names=1)
  endpoint_pvals <- endpoint_pvals[which(endpoint_pvals[,adj_pval_column_num]<0.05),adj_pval_column_num,drop=F]
...
for (c in cancer_list) {
  ...
  cancer_file <- paste0('../../',project,'_',c,'/metapcna/',c,'_significant_pval_differential_genes_',end,'.csv')
  cluster_file <- paste0('../../',project,'_',c,'/metapcna/',c,'_significant_cox_cluster_differential_genes_',end,'.csv')
  age_file <- paste0('../../',project,'_',c,'/metapcna/',c,'_significant_cox_age_differential_genes_',end,'.csv')
  gender_file <- paste0('../../',project,'_',c,'/metapcna/',c,'_significant_cox_gender_differential_genes_',end,'.csv')
  metapcna_file <- paste0('../../',project,'_',c,'/metapcna/',c,'_significant_cox_metapcna_differential_genes_',end,'.csv')
  # read and fill in cancer significant endpoint genes; use adjusted p-value
  sig_pval_genes <- get_sig_genes(cancer_file,5)
  ...
  sig_metapcna_genes <- get_sig_genes(metapcna_file) # cox gender pvalues
  sig_metapcna_num <- nrow(sig_metapcna_genes)
  metapcna_df[c,rownames(sig_metapcna_genes)] <- sig_metapcna_genes$adjpval_metapcna
  
  # fill in effect data frame
  # files for each effect (promoting and inhibiting)
  pro_file <- paste0('../../',project,'_',c,'/metapcna/',end,'_promoting_genes_protein_complexes.csv')
  inh_file <- paste0('../../',project,'_',c,'/metapcna/',end,'_inhibiting_genes_protein_complexes.csv')
...
write.csv(cancer_summary_df[order(cancer_summary_df$`Num Sig Epigenes`, decreasing = T),], paste0(project,'_metapcna_single_gene_',end,'_cancer_summary.csv'), row.names=T, quote=F)
write.csv(epigene_summary_df[order(epigene_summary_df$`Num Sig Cancer Types`, decreasing = T),], paste0(project,'_metapcna_single_gene_',end,'_epigene_summary.csv'), row.names=T, quote=F)
...
pval_heatmap(pval_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_clust.png'), 'P-values')
pval_heatmap(cluster_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_cluster_clust.png'),'Cox Cluster P-values')
pval_heatmap(age_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_age_clust.png'),'Cox Age P-values')
pval_heatmap(gender_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_gender_clust.png'),'Cox Gender P-values')
pval_heatmap(metapcna_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_metapcna_clust.png'),'Cox metapcna P-values')
...
pval_heatmap_no_cluster(pval_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_no_clust.png'), 'P-values')
pval_heatmap_no_cluster(cluster_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_cluster_no_clust.png'),'Cox Cluster P-values')
pval_heatmap_no_cluster(age_df, paste0(project,'_metapcna_single_gene_',end,'_adj_pvalues_cox_age_no_clust.png'),'Cox Age P-values')
pval_heatmap_no_cluster(gender_df, paste0(project,'_metapcna_single_gene_',end,'_adj_pvalues_cox_gender_no_clust.png'),'Cox Gender P-values')
pval_heatmap_no_cluster(metapcna_df, paste0(project,'_metapcna_single_gene_',end,'_adj_pvalues_cox_metapcna_no_clust.png'),'Cox metapcna P-values')
...
pval_heatmap_desc(pval_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_descending_orders.png'), 'P-values')
pval_heatmap_desc(cluster_df, paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_cluster_descending_orders.png'),'Cox Cluster P-values')
pval_heatmap_desc(age_df, paste0(project,'_metapcna_single_gene_',end,'_adj_pvalues_cox_age_descending_orders.png'),'Cox Age P-values')
pval_heatmap_desc(gender_df, paste0(project,'_metapcna_single_gene_',end,'_adj_pvalues_cox_gender_descending_orders.png'),'Cox Gender P-values')
pval_heatmap_desc(metapcna_df, paste0(project,'_metapcna_single_gene_',end,'_adj_pvalues_cox_metapcna_descending_orders.png'),'Cox metapcna P-values')


# write p-value table
write.csv(pval_df, file = paste0(project,'_metapcna_single_gene_',end,'_adjpvalues.csv'),row.names=T, quote=F)
write.csv(pval_df[,which(apply(is.na(pval_df),2,sum)<4)], file = paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_filtered.csv'),row.names=T, quote=F)
write.csv(cluster_df, file = paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_cluster.csv'),row.names=T, quote=F)
write.csv(cluster_df[,which(apply(is.na(cluster_df),2,sum)<4)], file = paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_cluster_filtered.csv'),row.names=T, quote=F)

write.csv(age_df, file = paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_age.csv'),row.names=T, quote=F)
write.csv(gender_df, file = paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_gender.csv'),row.names=T, quote=F)
write.csv(metapcna_df, file = paste0(project,'_metapcna_single_gene_',end,'_adjpvalues_cox_metapcna.csv'),row.names=T, quote=F)
...
# heatmap
png(paste0(project,'_metapcna_single_gene_effect_',end,'.png'),units='in',width=7,height=7,res=500)
...
dev.off()
# write effect table
write.csv(effect_df,paste0(project,'_metapcna_single_gene_effect_',end,'.csv'),row.names=T,quote=F)

#######
# Write Excel Sheets
#######
# Load effects df of survminer results prior to metapcna correction and pvalue dataframes
premeta_effect <- read.csv(paste0('../',project,'_single_gene_effect_',end,'.csv'),row.names=1)
premeta_pval <- read.csv(paste0('../',project,'_single_gene_',end,'_adjpvalues.csv'), row.names=1)
premeta_cluster <- read.csv(paste0('../',project,'_single_gene_',end,'_adjpvalues_cox_cluster.csv'), row.names=1)
premeta_age <- read.csv(paste0('../',project,'_single_gene_',end,'_adjpvalues_cox_age.csv'), row.names=1)
premeta_gender <- read.csv(paste0('../',project,'_single_gene_',end,'_adjpvalues_cox_gender.csv'), row.names=1)


library(stringr)
write_excel_sheet <- function(pval_df, premeta_df, filename, val_type){
  if(sum(is.na(pval_df))==(nrow(pval_df)*ncol(pval_df))){
    return('df contains all NA values')
  }
  # pval_df
  ...
  endpoint_effect_style <- function(value_name){
    ...
    eff <- premeta_effect[row_ind,col_ind]
    premeta_pval <- premeta_df[row_ind,col_ind]
    ...
    if (eff==1 && !na_val){
      ...
      if(eff==1 && !is.na(premeta_pval)){
        setCellStyle(cell=cells[[value_name]],cellStyle = cs_promote)
      } else if (eff==-1 && !is.na(premeta_pval)){
        setCellStyle(cell=cells[[value_name]],cellStyle = cs_inhibit)
      }
    }
  }
...
write_excel_sheet(pval_df, premeta_pval, paste0(project,'_metapcna_single_gene_adjpvalue_and_',end,'_effect.xlsx'), 'P-values')
write_excel_sheet(cluster_df, premeta_cluster, paste0(project,'_metapcna_single_gene_adjpvalue_cox_cluster_and_',end,'_effect.xlsx'), 'Cox Cluster P-values')
write_excel_sheet(age_df, premeta_age, paste0(project,'_metapcna_single_gene_adjpvalue_cox_age_and_',end,'_effect.xlsx'), 'Cox Age P-values')
write_excel_sheet(gender_df, premeta_gender, paste0(project,'_metapcna_single_gene_adjpvalue_cox_gender_and_',end,'_effect.xlsx'), 'Cox Gender P-values')
# write_excel_sheet(metapcna_df, paste0(project,'_metapcna_single_gene_adjpvalue_cox_metpcna_and_',end,'_effect.xlsx'), 'Cox metapcna P-values')


# count number of genes significant both before and after metapcna
premeta_epigene_summary_df <- read.csv(paste0('../',project,'_single_gene_', end, '_epigene_summary.csv'),row.names=1)

prog_gene_comp<- merge(epigene_summary_df, premeta_epigene_summary_df, by=0)
rownames(prog_gene_comp) <- prog_gene_comp$Row.names
prog_gene_comp <- prog_gene_comp%>% select(Num.Sig.Cancer.Types.with.Sig.Cox.Cluster, `Num Sig Cancer Types with Sig Cox Cluster`)
colnames(prog_gene_comp) <- c('Num_Cancer_Types_with_sig_cox_cluster','Num_Cancer_Types_with_sig_cox_cluster_metapcna')
prog_gene_comp$percent_preserved <- prog_gene_comp$Num_Cancer_Types_with_sig_cox_cluster_metapcna / 
  prog_gene_comp$Num_Cancer_Types_with_sig_cox_cluster
prog_gene_comp <- prog_gene_comp %>% arrange(desc(Num_Cancer_Types_with_sig_cox_cluster),desc(percent_preserved))
prog_gene_comp <- na.omit(prog_gene_comp)

write.csv(prog_gene_comp, paste0(project,'_single_gene_prognostic_comparison.csv'))
